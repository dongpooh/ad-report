...
# --- 사이드바 메뉴 ---
menu = st.sidebar.radio("📊 메뉴 선택", ["💰 마진현황", "SSP 리포트", "DSP 리포트", "📋 매체정산"])
...
if menu == "💰 마진현황":
    ...
    if st.button("🔍 조회하기", type="primary", key="m_btn"):
        if not file_erp or (not file_console1 and not file_console2):
            ...
            # 탭
            tab_normal, tab_split, tab_iwol, tab_nocode, tab_erp_only, tab_manual = st.tabs([
                "✅ 정상","⚠️ 분리필요","🔄 이월","📝 ERP코드 미등록","📦 ERP에만 존재","🚫 수기정산"])

            with tab_normal:
                if not df_normal.empty:
                    df_nv = df_normal[df_normal['매체사코드'].notna()].copy()
                    con_fee = df_con.groupby('ERP 매체코드', as_index=False)['운영수수료(KRW)'].sum().rename(columns={'운영수수료(KRW)':'콘솔_수수료합'})
                    df_nv = pd.merge(df_nv, con_fee, on='ERP 매체코드', how='left')
                    df_nv['콘솔_수수료합'] = df_nv['콘솔_수수료합'].fillna(0).astype(int)
                    df_nv['ERP_마진'] = df_nv['매체수수료 합계(KRW)'].fillna(0).astype(int)
                    df_nv['차이'] = df_nv['ERP_마진'] - df_nv['콘솔_수수료합']
                    df_nv['상태_chk'] = df_nv.apply(lambda r: '✅' if r['차이']==0 else f"⚠️ 차이 {r['차이']:,}원", axis=1)
                    if '비고_이월' in df_nv.columns:
                        df_nv.loc[df_nv['비고_이월'].notna(),'상태_chk'] = df_nv.loc[df_nv['비고_이월'].notna(),'상태_chk'] + ' (이월합산)'

                    st.success(f"✅ {len(df_nv)}건 정상")
                    col_f1, col_f2 = st.columns(2)
                    with col_f1:
                        flt = st.radio("필터:",["전체","✅ 일치만","⚠️ 차이만"], horizontal=True, key="nf")
                    with col_f2:
                        srt = st.radio("정렬:",["마진 높은순","마진 낮은순","차이 큰순"], horizontal=True, key="ns")

                    disp = df_nv[['회사명','ERP 매체코드','회사키','ERP_마진','콘솔_수수료합','차이','상태_chk']].copy()
                    disp = disp.rename(columns={'ERP_마진':'마진(ERP)','콘솔_수수료합':'콘솔 수수료','상태_chk':'상태'})
                    if flt=="✅ 일치만": disp = disp[disp['상태']=='✅']
                    elif flt=="⚠️ 차이만": disp = disp[disp['상태']!='✅']
                    if srt=="마진 높은순": disp = disp.sort_values('마진(ERP)', ascending=False)
                    elif srt=="마진 낮은순": disp = disp.sort_values('마진(ERP)')
                    elif srt=="차이 큰순": disp = disp.sort_values('차이', key=abs, ascending=False)
                    ...
                    disp['차이'] = disp['차이'].apply(lambda x: f"{x:,}")
                    st.table(disp.reset_index(drop=True))
                    t_m = df_nv['ERP_마진'].sum(); t_c = df_nv['콘솔_수수료합'].sum()
                    st.markdown(f"---\n**합계** | 마진(ERP): **{t_m:,}** | 콘솔 수수료: **{t_c:,}** | 차이: **{t_m-t_c:,}**")
            ...
            with tab_iwol:
                if not df_iwol.empty:
                    df_iwol['순매체비(KRW)'] = df_iwol['순매체비(KRW)'].apply(to_numeric_m)
                    df_iwol['운영수수료(KRW)'] = df_iwol['운영수수료(KRW)'].apply(to_numeric_m)
                    iw = df_iwol.sort_values('기준월', ascending=False).drop_duplicates(subset=['회사명'], keep='first')
                    st.info(f"🔄 {len(iw)}건 이월")
                    d = iw[['회사명','ERP 매체코드','회사키']].copy()
                    d['ERP 매체코드'] = d['ERP 매체코드'].apply(lambda x: '코드없음' if x in ['-','','nan'] else x)
                    d['콘솔 매체비'] = iw['순매체비(KRW)'].astype(int).apply(lambda x: f"{x:,}")
                    d['콘솔 수수료'] = iw['운영수수료(KRW)'].astype(int).apply(lambda x: f"{x:,}")
                    st.table(d.reset_index(drop=True))
                else: st.info("해당 없음")
            ...
            with tab_nocode:
                if not df_no_code.empty:
                    ...
                    df_no_code['운영수수료(KRW)'] = df_no_code['운영수수료(KRW)'].apply(to_numeric_m)
                    nc = df_no_code.groupby(['회사명','회사키'], as_index=False).agg({'순매체비(KRW)':'sum','운영수수료(KRW)':'sum'})
                    st.warning(f"📝 {len(nc)}건 ERP코드 없음")
                    d = nc[['회사명','회사키']].copy()
                    d['콘솔 매체비'] = nc['순매체비(KRW)'].astype(int).apply(lambda x: f"{x:,}")
                    d['콘솔 수수료'] = nc['운영수수료(KRW)'].astype(int).apply(lambda x: f"{x:,}")
                    st.table(d.reset_index(drop=True))
...
# =========================================================================
    ...
    SETTLEMENT_DIR = Path("data/settlement")
    SETTLEMENT_DIR.mkdir(parents=True, exist_ok=True)
    MAPPING_FILE = SETTLEMENT_DIR / "erp_mapping.json"
    ...
    def save_mapping_s(mapping):
        pass  # 배포 환경에서는 파일 저장 안 함

    st.header("📋 매체정산 시트 생성")
    st.caption("ERP + 콘솔 파일을 올리면, 신규 월 시트 데이터가 담긴 엑셀을 생성합니다.")

    target_month = st.text_input("생성할 월 (YYYYMM)", value="202607")
    target_month_int = int(target_month) if target_month.isdigit() else 202607
    y_t, m_t = int(target_month[:4]), int(target_month[4:6])
    m_t -= 1
    if m_t == 0: m_t = 12; y_t -= 1
    prev_month_int = y_t * 100 + m_t

    st.markdown("---")
    st.subheader("📤 파일 업로드")

    PREV_KEYS_FILE = SETTLEMENT_DIR / f"prev_keys_{prev_month_int}.json"
    BASE_DATA_FILE = SETTLEMENT_DIR / f"base_data_{prev_month_int}.json"
    prev_month_keys = set()

    if PREV_KEYS_FILE.exists() and BASE_DATA_FILE.exists():
        with open(PREV_KEYS_FILE, "r", encoding="utf-8") as f: prev_month_keys = set(json.load(f))
        st.success(f"✅ 캐시 사용: 전월({prev_month_int}) 업체 **{len(prev_month_keys)}개**")
        base_file_s = None
        use_cache_s = True
    else:
        st.warning(f"⚠️ 기준 데이터가 없습니다. 매체정산 엑셀을 올려주세요 (최초 1회).")
        base_file_s = st.file_uploader("① 매체정산 엑셀 (최초 1회)", type=["xlsx"], key="s_base")
        use_cache_s = False

    col_s1, col_s2, col_s3 = st.columns(3)
    with col_s1: erp_file_s = st.file_uploader("② ERP 파일", type=["xlsx"], key="s_erp")
    with col_s2: early_file_s = st.file_uploader("③ 조기 정산", type=["xlsx"], key="s_early")
    with col_s3: normal_file_s = st.file_uploader("④ 일반 정산", type=["xlsx"], key="s_normal")

    if not use_cache_s and not base_file_s:
        st.info("👆 매체정산 엑셀을 올려주세요.")
        st.stop()
    if not (erp_file_s or early_file_s or normal_file_s):
        st.info("👆 ERP 또는 콘솔 파일을 올려주세요.")
        st.stop()
    ...
    if not use_cache_s and base_file_s:
        wb_base = openpyxl.load_workbook(base_file_s, data_only=True, read_only=True)
        target_sheets = [s for s in wb_base.sheetnames if "매체정산 전체 내역_" in s and "_Roman" not in s and "_Derek" not in s]
        if not target_sheets: st.error("❌ 시트 못 찾음"); st.stop()
        latest_sheet = target_sheets[0]
        st.info(f"📌 읽는 시트: **{latest_sheet}**")
        ws = wb_base[latest_sheet]
        base_rows_all = []
        for row in ws.iter_rows(min_row=3, values_only=True):
            if row[0] is None and row[1] is None: continue
            try: key = str(int(float(row[1]))) if row[1] else ""
            except: key = str(row[1] or "").strip()
            try: settle_month = str(int(float(row[2]))) if row[2] else ""
            except: settle_month = str(row[2] or "").strip()
            if settle_month == str(prev_month_int): prev_month_keys.add(key)
            base_rows_all.append({
                "회사명": str(row[0]).strip() if row[0] else "", "회사키": key,
                "정산월": settle_month, "환율": to_num_s(row[3]),
                "정산방식": str(row[4]) if row[4] else "",
                "매체비_KRW": to_num_s(row[5]), "운영수수료_KRW": to_num_s(row[6]),
                "세금계산서발행액_KRW": to_num_s(row[7]),
                "통합발행_KRW": to_num_s(row[8]) if row[8] else "",
                "이월": str(row[9]).strip() if row[9] else "", "비고": str(row[10]).strip() if row[10] else ""
            })
        wb_base.close()
        with open(PREV_KEYS_FILE, "w", encoding="utf-8") as f: json.dump(list(prev_month_keys), f)
        with open(BASE_DATA_FILE, "w", encoding="utf-8") as f: json.dump(base_rows_all, f, ensure_ascii=False)
        st.success(f"✅ 기준 데이터 저장: **{len(prev_month_keys)}개** 전월 업체 / **{len(base_rows_all)}행**")

    # ERP 처리 전에 콘솔에서 매핑 먼저 추출
    mapping = {}
    for label, f in [("조기", early_file_s), ("일반", normal_file_s)]:
        if f is None: continue
        cdf_map = pd.read_excel(f, engine="openpyxl")
        cdf_map.columns = cdf_map.columns.astype(str).str.strip()
        cols_map = list(cdf_map.columns)
        erp_code_col_m = next((c for c in cols_map if "ERP" in c and ("코드" in c or "code" in c.lower())), None)
        key_col_m = next((c for c in cols_map if "회사키" in c), None)
        name_col_m = next((c for c in cols_map if "회사명" in c), None)
        if erp_code_col_m and key_col_m:
            for _, row in cdf_map.iterrows():
                ec = str(row[erp_code_col_m]).replace(".0", "").strip() if pd.notna(row[erp_code_col_m]) else ""
                ck = str(row[key_col_m]).replace(".0", "").strip() if pd.notna(row[key_col_m]) else ""
                cn = str(row[name_col_m]).strip() if name_col_m and pd.notna(row[name_col_m]) else ""
                if ec and ec != "-" and ck:
                    mapping[ec] = {"회사키": ck, "회사명": cn}

    # ERP 처리
    erp_result, erp_duplicates, erp_unmapped, erp_keys_added = [], [], [], set()

    if erp_file_s:
        erp_df = pd.read_excel(erp_file_s, engine="openpyxl")
        erp_df.columns = erp_df.columns.astype(str).str.strip()
        cols = list(erp_df.columns)
        code_col = next((c for c in cols if "매체사코드" in c or "매체코드" in c), None)
        name_col = next((c for c in cols if "매체사" in c and "명" in c), None)
        fee_col = next((c for c in cols if "정산매체비" in c and "KRW" in c), None)
        if not fee_col: fee_col = next((c for c in cols if "매체비 합계(KRW)" in c), None)
        if not fee_col: fee_col = next((c for c in cols if "매체비" in c and "합계" in c), None)
        if not fee_col: fee_col = next((c for c in cols if "매체비" in c), None)
        commission_col = next((c for c in cols if "수수료" in c and "합계" in c), None)
        invoice_col = next((c for c in cols if "계산서발행구분" in c), None)

        if code_col:
            for _, row in erp_df.iterrows():
                erp_code = str(row[code_col]).replace(".0", "").strip() if pd.notna(row[code_col]) else ""
                erp_name = str(row[name_col]).strip() if name_col and pd.notna(row[name_col]) else ""
                fee = to_num_s(row[fee_col]) if fee_col else 0
                comm = to_num_s(row[commission_col]) if commission_col else 0
                inv_type = str(row[invoice_col]).strip() if invoice_col and pd.notna(row[invoice_col]) else ""
                if not erp_code or fee == 0: continue
                if erp_code in mapping:
                    company_key = mapping[erp_code]["회사키"]
                    company_name = mapping[erp_code]["회사명"]
                else:
                    erp_unmapped.append({"ERP코드": erp_code, "매체사명": erp_name, "매체비": fee})
                    continue
                method = "GROSS" if ("총액" in inv_type or "GROSS" in inv_type.upper()) else "NET"
                invoice_amt = fee - comm if method == "NET" and fee > comm else fee
                erp_result.append({
                    "회사명": company_name, "회사키": company_key,
                    "정산월": target_month_int, "환율": 0, "정산방식": method,
                    "매체비_KRW": fee, "운영수수료_KRW": comm,
                    "세금계산서발행액_KRW": invoice_amt, "통합발행_KRW": invoice_amt,
                    "이월": "발행완료", "비고": ""
                })
            key_counts = Counter(r["회사키"] for r in erp_result)
            dup_keys = {k for k, cnt in key_counts.items() if cnt >= 2}
            erp_duplicates = [r for r in erp_result if r["회사키"] in dup_keys]
            erp_result = [r for r in erp_result if r["회사키"] not in dup_keys]
            erp_keys_added = {r["회사키"] for r in erp_result}

    # 콘솔 처리
    console_result = []
    for label, f in [("조기", early_file_s), ("일반", normal_file_s)]:
        if f is None: continue
        cdf = pd.read_excel(f, engine="openpyxl")
        cdf.columns = cdf.columns.astype(str).str.strip()
        cols = list(cdf.columns)
        erp_code_col = next((c for c in cols if "ERP" in c and ("코드" in c or "code" in c.lower())), None)
        key_col = next((c for c in cols if "회사키" in c), None)
        name_col = next((c for c in cols if "회사명" in c), None)
        month_col = next((c for c in cols if "기준월" in c or "정산월" in c), None)
        rate_col = next((c for c in cols if "환율" in c), None)
        method_col = next((c for c in cols if "정산방식" in c), None)
        fee_col = next((c for c in cols if "순매체비" in c and "KRW" in c), None)
        if not fee_col: fee_col = next((c for c in cols if "매체비" in c and "KRW" in c.upper()), None)
        comm_col = next((c for c in cols if "운영수수료" in c and "KRW" in c.upper()), None)
        if not comm_col: comm_col = next((c for c in cols if "운영수수료(KRW)" in c), None)
        invoice_col_c = next((c for c in cols if "세금계산서" in c or "발행액" in c), None)
        # 매핑 갱신
        if erp_code_col and key_col:
            for _, row in cdf.iterrows():
                ec = str(row[erp_code_col]).replace(".0", "").strip() if pd.notna(row[erp_code_col]) else ""
                ck = str(row[key_col]).replace(".0", "").strip() if pd.notna(row[key_col]) else ""
                cn = str(row[name_col]).strip() if name_col and pd.notna(row[name_col]) else ""
                if ec and ec != "-" and ck: mapping[ec] = {"회사키": ck, "회사명": cn}
        if not month_col or not key_col: continue
        for _, row in cdf.iterrows():
            month_val = int(to_num_s(row[month_col])) if pd.notna(row[month_col]) else 0
            if month_val != target_month_int: continue
            key = str(row[key_col]).replace(".0", "").strip() if pd.notna(row[key_col]) else ""
            if not key or key in erp_keys_added: continue
            if key not in prev_month_keys: continue
            fee = to_num_s(row[fee_col]) if fee_col else 0
            if fee == 0: continue
            company = str(row[name_col]).strip() if name_col and pd.notna(row[name_col]) else ""
            rate = to_num_s(row[rate_col]) if rate_col else 0
            method = str(row[method_col]).strip() if method_col and pd.notna(row[method_col]) else "NET"
            comm = to_num_s(row[comm_col]) if comm_col else 0
            invoice = to_num_s(row[invoice_col_c]) if invoice_col_c else 0
            console_result.append({
                "회사명": company, "회사키": key, "정산월": target_month_int,
                "환율": rate, "정산방식": method, "매체비_KRW": fee,
                "운영수수료_KRW": comm, "세금계산서발행액_KRW": invoice,
                "통합발행_KRW": "", "이월": "미발행", "비고": ""
            })
            erp_keys_added.add(key)

    save_mapping_s(mapping)

    # 환율 반영
    default_rate = 0
    for r in console_result:
        if r.get("환율") and r["환율"] != 0: default_rate = r["환율"]; break
    for r in erp_result:
        if r["환율"] == 0: r["환율"] = default_rate
    for r in erp_duplicates:
        if r["환율"] == 0: r["환율"] = default_rate

    # 결과 생성
    all_rows = erp_result + console_result
    output_cols = ["회사명", "회사키", "정산월", "환율", "정산방식", "매체비_KRW", "운영수수료_KRW", "세금계산서발행액_KRW", "통합발행_KRW", "이월", "비고"]

    col_s1, col_s2, col_s3, col_s4 = st.columns(4)
    col_s1.metric("ERP 추가", f"{len(erp_result)}건")
    col_s2.metric("콘솔 추가", f"{len(console_result)}건")
    col_s3.metric("합계", f"{len(all_rows)}건")
    col_s4.metric("중복(검열)", f"{len(erp_duplicates)}건")

    if all_rows:
        base_data_file = BASE_DATA_FILE
        base_rows = []
        if base_data_file.exists():
            with open(base_data_file, "r", encoding="utf-8") as f: base_rows = json.load(f)

        # ERP 발행 업체의 기존 "미발행" → "이월"
        erp_only_keys = {r["회사키"] for r in erp_result}
        for row in base_rows:
            if row.get("회사키", "") in erp_only_keys and row.get("이월") == "미발행":
                row["이월"] = "이월"

        # 회사키별 마지막 행 뒤에 삽입
        new_by_key = {}
        for r in all_rows:
            new_by_key.setdefault(r["회사키"], []).append(r)

        combined_rows, seen_keys = [], set()
        for i, row in enumerate(base_rows):
            combined_rows.append(row)
            key = row.get("회사키", "")
            next_key = base_rows[i + 1].get("회사키", "") if i + 1 < len(base_rows) else ""
            if key != next_key and key in new_by_key and key not in seen_keys:
                combined_rows.extend(new_by_key[key])
                seen_keys.add(key)

        for key, rows in new_by_key.items():
            if key not in seen_keys: combined_rows.extend(rows)

        # 업체별 정산월 정렬
        from itertools import groupby
        sorted_combined = []
        for _, group in groupby(combined_rows, key=lambda r: r.get("회사키", "")):
            gl = sorted(list(group), key=lambda r: str(r.get("정산월", "0")))
            sorted_combined.extend(gl)
        combined_rows = sorted_combined

        result_df = pd.DataFrame(combined_rows)[output_cols]
        st.subheader(f"✅ {target_month} 시트 ({len(combined_rows)}행)")
        new_df = pd.DataFrame(all_rows)[output_cols]
        st.markdown(f"**신규 {len(all_rows)}건:**")
        st.dataframe(new_df, use_container_width=True, hide_index=True, height=300)

        output_buf = io.BytesIO()
        with pd.ExcelWriter(output_buf, engine="openpyxl") as writer:
            result_df.to_excel(writer, sheet_name=f"매체정산_{target_month}", index=False)
            if erp_duplicates:
                pd.DataFrame(erp_duplicates)[output_cols].to_excel(writer, sheet_name="검열_중복", index=False)
            if erp_unmapped:
                pd.DataFrame(erp_unmapped).to_excel(writer, sheet_name="매핑안됨", index=False)
        st.download_button(f"📥 엑셀 다운로드 ({len(combined_rows)}행)", output_buf.getvalue(),
                          f"매체정산_{target_month}.xlsx",
                          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                          type="primary", use_container_width=True)

    if erp_duplicates:
        st.subheader(f"⚠️ 검열 (중복): {len(erp_duplicates)}건")
        st.dataframe(pd.DataFrame(erp_duplicates)[["회사명", "회사키", "정산방식", "매체비_KRW", "운영수수료_KRW"]], use_container_width=True, hide_index=True)
    if erp_unmapped:
        st.subheader(f"❌ 매핑 안 됨: {len(erp_unmapped)}건")
        st.dataframe(pd.DataFrame(erp_unmapped), use_container_width=True, hide_index=True)

    st.caption(f"매핑: {len(mapping)}건 | 전월 업체: {len(prev_month_keys)}개")
