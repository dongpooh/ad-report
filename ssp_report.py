...
# --- 사이드바 메뉴 ---
menu = st.sidebar.radio("📊 메뉴 선택", ["매체정산 📋", "마진현황 💰", "SSP 리포트 📈", "DSP 리포트 📊"])
...
# =========================================================================
    ...
    st.markdown("---")
    st.subheader("📤 파일 업로드")
    st.markdown("아래 4개 파일을 올려주세요. 처리 후 엑셀을 다운받아 시트에 복붙하면 됩니다.")

    prev_month_keys = set()

    base_file_s = st.file_uploader("① [Monetize] 매체비 내역 엑셀 (기준 시트 — 두 번째 탭만 읽음)", type=["xlsx"], key="s_base")
    ...
    if base_file_s:
        ...
        wb_base.close()
        st.success(f"✅ 기준 데이터 로드: 전월({prev_month_int}) 업체 **{len(prev_month_keys)}개** / 전체 **{len(base_rows_all)}행**")
    ...
    if all_rows:
        base_rows = base_rows_all if 'base_rows_all' in dir() else []
        ...
        # 숫자 컬럼에 콤마 포맷
        num_cols = ["매체비_KRW", "운영수수료_KRW", "세금계산서발행액_KRW", "통합발행_KRW", "환율"]
        display_new = new_df.copy()
        for c in num_cols:
            if c in display_new.columns:
                display_new[c] = display_new[c].apply(lambda x: f"{x:,.0f}" if isinstance(x, (int, float)) and x != 0 else x)
        st.markdown(f"**신규 {len(all_rows)}건:**")
        st.dataframe(display_new, use_container_width=True, hide_index=True, height=300)
    ...
    if erp_duplicates:
        ...
        dup_disp = pd.DataFrame(erp_duplicates)[["회사명", "회사키", "정산방식", "매체비_KRW", "운영수수료_KRW"]].copy()
        dup_disp["매체비_KRW"] = dup_disp["매체비_KRW"].apply(lambda x: f"{x:,.0f}" if isinstance(x, (int, float)) else x)
        dup_disp["운영수수료_KRW"] = dup_disp["운영수수료_KRW"].apply(lambda x: f"{x:,.0f}" if isinstance(x, (int, float)) else x)
        st.dataframe(dup_disp, use_container_width=True, hide_index=True)
    if erp_unmapped:
        st.subheader(f"❌ 매핑 안 됨: {len(erp_unmapped)}건")
        unmap_disp = pd.DataFrame(erp_unmapped).copy()
        unmap_disp["매체비"] = unmap_disp["매체비"].apply(lambda x: f"{x:,.0f}" if isinstance(x, (int, float)) else x)
        st.dataframe(unmap_disp, use_container_width=True, hide_index=True)
